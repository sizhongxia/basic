...
<template>
  <div>
    ...
    <Modal
      ...
      <div style="width:100%;height:260px;overflow: auto;text-align: left;">
        <Form :model="formObj" :label-width="190" ref="baseForm">
          <Row>
              ...
              <Col span="12">
                  <FormItem label="月份">
                    <InputNumber :step="1" :min="1" :max="12" :disabled="formObj.resId != ''" style="width: 200px;" v-model="formObj.dateMonth" placeholder="请输入月份"></InputNumber>
                  </FormItem>
              </Col>
              <Col span="12">
                <FormItem label="农产品">
      ...
      <Spin size="large" fix v-if="submiting || loadingTypes"></Spin>
      <div slot="footer">
...
<script>
import { loadMoaWpapMonth, upinsertWpapMonth, deleteWpapMonth, allMoaTypes } from '@/api/moa'
export default {
  data () {
    return {
      searchYear: '',
  ...
  methods: {
    load () {
      ...
      let size = this.size
      const _this = this
      loadMoaWpapMonth({ searchYear, searchMonth, searchTypeId, page, size }).then(res => {
        _this.loading = false
    ...
    handleDelete (params) {
      const _this = this
      _this.deleting = true
      deleteWpapMonth({ resultId: params.row.resId }).then(res => {
    ...
    submitBaseFormHandle () {
      ...
      upinsertWpapMonth(_this.formObj).then(res => {
